...
pub fn test_in_two_rows_varchar_and_update() {
    ...
    let _ = sql_executor.parse_command("INSERT table1 VALUES (1, \'fabiano\', \'martins\')");
    let _ = sql_executor.parse_command("INSERT table1 VALUES (2, \'fabiano2\', \'martins2\')");

    let result_set = sql_executor.parse_command("UPDATE table1 SET name1 = \"fabiano3\"");

    assert!(matches!(result_set, Ok(_result_set)));

    let result_set_select = sql_executor.parse_command("SELECT * FROM table1 WHERE name = \"fabiano3\"");
